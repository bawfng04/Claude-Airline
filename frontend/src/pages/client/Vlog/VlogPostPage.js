...
const useAuth = () => {
    ...
    if (loggedIn) {
        return {
            ...
            user: { id: 2, GIVEN_NAME: 'Demo User', avatarUrl: null },
            token: "YOUR_MOCK_AUTH_TOKEN_REPLACE_ME"
        };
    }
                ...
                <div className="offer-content">
                    ...
                    <form className="offer-form" onSubmit={handleSubmit}>
                        <div className="form-row">
                            <input
                                type="email"
                                className="offer-input text-sm sm:text-base p-2 sm:p-3"
...
const AddVlogCommentForm = ({ postId, authToken, onCommentAdded, isSubmittingForm, setIsSubmittingForm, isAuthenticatedFromParent, loggedInUser }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!isAuthenticatedFromParent && !guestFullName.trim()) { setFormError("Please enter your full name to comment as a guest."); return; }
        setIsSubmittingForm(true);
        const ADD_COMMENT_API = `${API_URL}/vlogComment/createComment/${postId}`;
        const payload = { comment: comment.trim(), rating: rating > 0 ? rating : null };
        if (!isAuthenticatedFromParent) { payload.guest_name = guestFullName.trim(); }
        try {
            const headers = { 'Content-Type': 'application/json' };
            if (isAuthenticatedFromParent && authToken) { headers['Authorization'] = `Bearer ${authToken}`; }
            ...
            setFormSuccess("Your comment submitted and awaiting approval!");
            setRating(0); setComment(''); if (!isAuthenticatedFromParent) setGuestFullName('');
            if (onCommentAdded) { setTimeout(() => { setFormSuccess(null); onCommentAdded(); }, 2500); }
        } catch (err) { setFormError(err.message || "An unexpected error occurred."); }
        finally { setIsSubmittingForm(false); }
    };

    const avatarIcon = isAuthenticatedFromParent && loggedInUser?.avatarUrl
        ? <img src={loggedInUser.avatarUrl} alt={loggedInUser.GIVEN_NAME || 'User'} className="w-10 h-10 rounded-full object-cover flex-shrink-0" />
    ...
    return (
        <div className="bg-gray-50 p-4 sm:p-6 rounded-lg border border-gray-200 flex gap-3 sm:gap-4 items-start">
            <div className="mt-1">{avatarIcon}</div>
            <form onSubmit={handleSubmit} className="flex-grow space-y-4">
                {formError && <div className="text-red-600 bg-red-100 p-3 rounded-md border border-red-300 text-sm font-medium">{formError}</div>}
                {formSuccess && <div className="text-green-700 bg-green-100 p-3 rounded-md border border-green-400 text-sm font-medium">{formSuccess}</div>}
                {!isAuthenticatedFromParent && (
                    <input
                        type="text"
                        ...
                        placeholder="Your full name (required for guests)"
                        value={guestFullName}
                ...
                <textarea
                    ...
                    placeholder={isAuthenticatedFromParent ? `Share your thoughts, ${loggedInUser?.GIVEN_NAME || 'User'}...` : "Write a public comment..."}
                    required
                    disabled={isSubmittingForm}
                />
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-1">
                    <div className="flex items-center space-x-1">
                        ...
                        {[1, 2, 3, 4, 5].map((star) => (
                            <FaStar
                                key={star}
                                className={`cursor-pointer text-2xl sm:text-3xl transition-all duration-150 ease-in-out transform hover:scale-110
                                         ${(hoverRating || rating) >= star ? 'text-yellow-400' : 'text-gray-300 hover:text-yellow-300'}`}
                    ...
                    <button
                        type="submit"
                        disabled={isSubmittingForm || !comment.trim() || (!isAuthenticatedFromParent && !guestFullName.trim())}
...
const GalleryComponent = ({ images, title }) => (
    <section aria-label="Post Gallery" className="py-8 md:py-12 px-2 sm:px-4">
                    ...
                    <div key={index} className="w-full aspect-square bg-gray-200 rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 group">
                        <img
                            ...
                            onError={(e) => {
                                ...
                                e.target.src = 'https://placehold.co/300x300/e2e8f0/94a3b8?text=Error';
...
const VlogPostPage = () => {
    ...
    const { isAuthenticated, user: loggedInUser, token: authToken } = useAuth();
            ...
            if (result.status === 200 && result.data) {
                setPost({
                    ...
                    featured_image: result.data.featured_image
                        ? (result.data.featured_image.startsWith('/') ? API_URL + result.data.featured_image : result.data.featured_image)
                        : 'https://placehold.co/1200x600/e2e8f0/94a3b8?text=Image+Not+Available',
    ...
    if (loading) return <div className="flex justify-center items-center min-h-screen bg-white"><FaSpinner className="animate-spin text-5xl sm:text-6xl text-red-600" /></div>;
    if (error) return (
    ...
    const shareSummary = post.introduction || '';
    const shareImage = post.featured_image || '';
    ...
    const paragraphs = post.main_content ? post.main_content.split('\n\n') : [];
    const canInterleaveContent = paragraphs.length === 4 && hasGalleryImages;

    return (
        <>
            <Helmet>
                <title>{`${post.title} | Claude Airlines Vlog`}</title>
                <meta name="description" content={post.introduction ? post.introduction.substring(0, 160) : (post.main_content ? post.main_content.substring(0,160) : '')} />
                ...
                <meta property="og:description" content={post.introduction ? post.introduction.substring(0, 200) : (post.main_content ? post.main_content.substring(0,200) : '')} />
                {isHeroImagePresent && <meta property="og:image" content={post.featured_image} />}
                <meta property="twitter:card" content="summary_large_image" />
                <meta property="twitter:url" content={shareUrl} />
                <meta property="twitter:title" content={post.title} />
                <meta property="twitter:description" content={post.introduction ? post.introduction.substring(0, 200) : (post.main_content ? post.main_content.substring(0,200) : '')} />
                {isHeroImagePresent && <meta property="twitter:image" content={post.featured_image} />}
            </Helmet>

            <div
                className="bg-white min-h-screen"
                style={{ paddingTop: 'var(--header-height, 70px)' }}
            >
                {isHeroImagePresent && (
                    <div className="relative w-full h-[calc(100vh-var(--header-height,70px)-0px)] max-h-[800px] sm:h-[calc(100vh-var(--header-height,70px)-0px)] text-white group selection:bg-red-500 selection:text-white">
                        <img
                        ...
                        <div className="relative z-10 h-full flex flex-col justify-center items-center text-center">
                            <div className="max-w-4xl w-full mx-auto px-4 sm:px-6 lg:px-8">
                                <h1 className="text-7xl sm:text-7xl md:text-7xl lg:text-8xl font-extrabold leading-tight [text-shadow:_2px_2px_4px_rgb(0_0_0_/_50%)]">
                                    {post.title}
                                </h1>
                                <hr className="my-5 sm:my-6 md:my-8 border-2 border-gray-400/60 max-w-xs sm:max-w-sm md:max-w-md mx-auto" />
                                {post.introduction && (
                                    <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl text-gray-100 italic leading-relaxed max-w-3xl mx-auto [text-shadow:_1px_1px_3px_rgb(0_0_0_/_40%)]">
                                        {post.introduction}
                                    </p>
                                )}
                            </div>
                ...
                <div className="w-full">
                            ...
                            {(post.main_content && post.main_content.trim() !== '') ? (
                                <div className="text-gray-800 selection:bg-red-100 selection:text-red-700">
                                    <div style={{ whiteSpace: 'pre-wrap' }} className="text-gray-700 text-base sm:text-lg leading-relaxed">
                                        {canInterleaveContent ? paragraphs.slice(0, 2).join('\n\n') : post.main_content}
                                    </div>
                                </div>
                            ) : (
                    ...
                    {canInterleaveContent && (
                        <GalleryComponent images={post.gallery_images} title={post.title} />
                    )}
                    ...
                    {!canInterleaveContent && hasGalleryImages && (
                        <GalleryComponent images={post.gallery_images} title={post.title} />
                    )}
                    ...
                    <div className="max-w-3xl mx-auto px-3 sm:px-4 lg:px-0">
                        <section aria-label="Share this post" className="py-5 my-5 border-t border-b border-gray-200">
                            <div className="flex items-center gap-x-4 text-sm">
                                ...
                                <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-blue-600 transition-colors" title="Share on Facebook">
                                    <FaFacebook size={24} />
                                </a>
                                <a href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(shareTitle)}`} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-sky-500 transition-colors" title="Share on Twitter">
                                    <FaTwitter size={24} />
                                </a>
                                <a href={`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(shareTitle)}&summary=${encodeURIComponent(shareSummary)}&source=Claude%20Airlines`} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-blue-600 transition-colors" title="Share on LinkedIn">
                                    <FaLinkedin size={22} />
                                </a>
                                {isHeroImagePresent && (
                                    <a href={`https://pinterest.com/pin/create/button/?url=${encodeURIComponent(shareUrl)}&media=${encodeURIComponent(shareImage)}&description=${encodeURIComponent(shareTitle)}`} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-red-600 transition-colors" title="Pin on Pinterest">
                                        <FaPinterest size={22} />
                                    </a>
                                )}
                                <a href={`https://api.whatsapp.com/send?text=${encodeURIComponent(shareTitle + " " + shareUrl)}`} target="_blank" rel="noopener noreferrer" data-action="share/whatsapp/share" className="text-gray-500 hover:text-green-500 transition-colors" title="Share on WhatsApp">
                                    <FaWhatsapp size={22} />
                                </a>
                                <a href={`https://www.reddit.com/submit?url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(shareTitle)}`} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-orange-500 transition-colors" title="Share on Reddit">
                                    <FaRedditAlien size={22} />
                                </a>
                                <a href={`mailto:?subject=${encodeURIComponent(shareTitle)}&body=${encodeURIComponent("Check out this vlog post: " + shareUrl)}`} className="text-gray-500 hover:text-gray-700 transition-colors" title="Share via Email">
                                    <FaEnvelope size={22} />
                                </a>
                            </div>
                        </section>

                        <section id="comments-section" aria-label="Comments" className="py-5">
                            <div className="mb-6 sm:mb-8">
                                <AddVlogCommentForm
                                    postId={post.id}
                                    authToken={authToken}
                                    onCommentAdded={fetchComments}
                                    isSubmittingForm={isSubmittingComment}
                                    setIsSubmittingForm={setIsSubmittingComment}
                                ...
                                comments.filter(c => c.is_approved).length > 0 ? (
                                    <div className="space-y-4 sm:space-y-5">
                                        {comments.filter(c => c.is_approved).map((comment) => (
                                            <div key={comment.id} className="flex gap-3 sm:gap-4 p-3 sm:p-4 bg-gray-50 border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200">
                                                { comment.user_avatar_url ?
                                                    <img src={comment.user_avatar_url.startsWith('/') ? API_URL + comment.user_avatar_url : comment.user_avatar_url} alt={comment.user_name || comment.guest_name} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover flex-shrink-0 mt-0.5" />
                                                ...
                                                <div className="flex-grow">
                                                    <div className="flex items-center justify-between mb-1">
                                                        ...
                                                        <span className="text-xs sm:text-sm text-gray-500 flex-shrink-0 ml-2">{formatCommentDate(comment.created_at)}</span>
                                                    </div>
                                                    {comment.rating && (
                                                        <div className="flex items-center mb-1 sm:mb-1.5">
                                                            {[...Array(5)].map((_, i) => (
                                                                <FaStar key={i} className={`text-sm ${i < comment.rating ? 'text-yellow-400' : 'text-gray-300'}`} />
                                                            ))}
                                                        </div>
                                                    )}
                                                    ...
                                                    <div className="flex items-center mt-2.5 pt-2.5 space-x-3 sm:space-x-4 border-t border-gray-100">
                                                        <button
                                                            ...
                                                            title={comment.userLiked ? "Unlike this comment" : "Like this comment"}
                                                            className={`flex items-center text-xs sm:text-sm py-1 px-2 rounded-md transition-colors duration-150 group disabled:opacity-50 disabled:cursor-wait ${
                                                                comment.userLiked ? 'text-red-600 bg-red-100 font-semibold' : 'text-gray-500 hover:text-red-600 hover:bg-red-50'
                                                            }`}
                                                        ...
                                                        >
                                                            {likingCommentId === comment.id ? (
                                                                ...
                                                                comment.userLiked ? (
                                                                    <FaThumbsUp className="mr-1 text-red-500 text-sm" />
                                                                ) : (
                                                                    <FaRegThumbsUp className="mr-1 group-hover:text-red-500 text-sm" />
                                                                )
                                                            )}
